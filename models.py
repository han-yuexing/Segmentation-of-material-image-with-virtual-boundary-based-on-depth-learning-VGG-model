...
def AtrousFCN_Resnet50_16s(input_shape = None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=7):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
    ...
    model = Model(img_input, x)
    weights_path = os.path.expanduser(os.path.join('~', '.keras/models/fcn_resnet50_weights_tf_dim_ordering_tf_kernels.h5'))
    #weights_path = '/notebooks/FCN_test/aurora_fcn2/Keras-FCN/Models/AtrousFCN_Resnet50_16s/checkpoint_weights.hdf5'
    model.load_weights(weights_path, by_name=True)
    return model
